...
class ScholarArticle(object):
    ...
    def as_csv(self, header=False, sep='|'):
        ...
        a = {'url': self.attrs['url'][0], 'title': self.attrs['title'][0],
             'url_citations': self.attrs['url_citations'][0],'excerpt':self.attrs['excerpt'][0]}
        ...
        title = a['title']
        url = a['url']
        url_citation = a['url_citations']
        excerpt=a['excerpt']
        return [title, url, url_citation,excerpt]
        # res = []
        # if header:
...
class ScholarArticleParser120726(ScholarArticleParser):
    ...
    def _ss_parse_article(self, art):
        self.article = ScholarArticle()
        # print("Inside parse_article ---> ")
        for tag in art:
            # print("Inside parse_article loop ---> \n", tag)
            # print("Inside parse_article loop ---> ")
            if tag.name == 'header' and self._tag_has_class(tag, 'search-result-header'):
                ...
                # print("Url ---> ", self.article['url'])
                # ytag = tag.find('ul', {'class': 'subhead'})
                # li_list = ytag.findAll('li',text=True)
                # print(li_list)
                # print("Year ---> ", li_list[len(li_list)-1])
                # print("Year ---> ", self.article['year'])
...
class ClusterScholarQuery(ScholarQuery):
    ...
    know about.
    """
    SCHOLAR_CLUSTER_URL = ScholarConf.GS_SCHOLAR_SITE + '/scholar?' \
                          + 'cluster=%(cluster)s' \
                          + '%(num)s'
...
def csv(querier, header=False, sep='|'):
    ...
    combine = []
    for art in articles:
        [title,url,url_citation,excerpt] = art.as_csv(header=header, sep=sep)
        # print(encode(result))
        if url!=None:
            combine.append([title,url,excerpt])
        elif url==None:
            combine.append([title,url_citation,excerpt])
        header = False
    return combine
...
def getResponseFromScopus(query, count):
    query = query.replace(' ', '+')
    ...
    SCOPUS_SCHOLAR_QUERY_URL = SCOPUS_SCHOLAR_SITE + '/search/scopus?' \
                               + 'query=title-abs-key(' + query + ')&SUBJAREA(COMP)&field=title,doi,coverDate&count=' + str(
        count)
...
def main(query):
    ...
    # parser = optparse.OptionParser(usage=None, formatter=fmt)
    ...
    # chromedriver='/home/gamechanger/chromedriver'
    # os.environ["webdriver.chrome.driver"] = chromedriver
    # gs_driver = webdriver.Chrome(chromedriver)
    # mas_driver = webdriver.Firefox()
    cnt = 0
    # print('input query')
    cl = query
    # print('no. of results')
    ...
    querier_scopus = getResponseFromScopus(cl, 11)
    ...
    for j in range(len(scopus_urls)):
        scopus.append([scopus_articles[j], scopus_urls[j]])
